      ...
      <div class="d-flex justify-space-between align-center flex-wrap gap-4">
        <div class="content">
          <div class="d-flex align-center gap-3">
            ...
            <div>
              <h3 class="font-weight-bold">Đối tác Sale</h3>
              <p class="text-medium-emphasis mb-0">
                Quản lý danh sách đối tác sale và chiết khấu
              </p>
            </div>
          </div>
        </div>
    ...
    <VCardItem>
      ...
      >
        ...
        <template #[`item.commission`]="{ item }">
          <VChip
            :color="getCommissionColor(item.commission)"
            variant="tonal"
            size="small"
          >
            {{ item.commission }}%
        ...
        <template #[`item.actions`]="{ item }">
          <div class="d-flex gap-1">
            <Link :href="route('partner.show', { partner: item.id })">
        ...
        <template #no-data>
          <div class="text-center py-8">
            ...
            <VBtn @click="addNewItem" color="primary">
              <VIcon icon="tabler-plus" class="me-2" />
...
const saleData = ref({ data: [], total: 0 });
...
const fetchData = async () => {
  // Chỉ fetch khi đang ở tab "0" (Sale)
  if (props.currentTab !== "0" && props.currentTab !== 0) return;
  ...
  try {
    ...
    const res = await axios.get(route("partner.loadDataSale"), { params });
    ...
    console.error("Lỗi khi tải dữ liệu Sale:", error);
  } finally {
    loading.value = false;
  }
};
...
watch(
  () => props.property_id,
  (val) => {
    console.log("Property ID changed:", val);
    fetchData();
  },
  { immediate: true }
);
...
watch([page, itemsPerPage], () => {
  if ((props.currentTab === "0" || props.currentTab === 0) && !loading.value) {
    fetchData();
  }
});
</script>
