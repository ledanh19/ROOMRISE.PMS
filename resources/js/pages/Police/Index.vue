  ...
  <section class="page">
    ...
    <div class="content-wrap">
      ...
      <article class="prose card">
        ...
        <footer class="doc-footer">© Roomrise</footer>
      </article>
    </div>
  </section>
</template>
...
onMounted(() => {
  ...
  // Sections theo dõi (ưu tiên <section id>, sau đó h2/h3/h4 có id)
  ...
  // Vị trí top của section (được cập nhật khi resize/content thay đổi)
  let positions = [];
  const HEADER_OFFSET = 90; // trùng với scroll-margin-top
  const VIEWPORT_ANCHOR = 0.35; // 35% từ đỉnh viewport (điểm tham chiếu)
  const HYSTERESIS = 24; // px: ngưỡng đổi mục để tránh nhảy
  ...
  // Tìm index section hiện tại theo điểm tham chiếu + hysteresis
  const computeIndex = () => {
    const refLine =
      window.scrollY + window.innerHeight * VIEWPORT_ANCHOR - HEADER_OFFSET;

    // tìm section có top <= refLine, lấy cái gần nhất
    let idx = -1;
    ...
    // hysteresis: chỉ đổi khi vượt qua khoảng cách tối thiểu
  ...
  const onResize = () => {
    measure();
    // giữ active ổn định sau khi đo lại
    const idx = computeIndex();
    activateByIndex(idx);
  };
  ...
  // Khởi tạo
  measure();
  onScroll(); // set active ban đầu

  window.addEventListener("scroll", onScroll, { passive: true });
  window.addEventListener("resize", onResize);

  // Nếu có thay đổi kích thước nội dung article, re-measure
  ...
  // Hash change: kích hoạt đúng mục nhưng vẫn dùng cơ chế mượt về sau
  const onHash = () => {
